...
# same five CSVs for every phase
DATASET_PATHS = [
    "./Dataset/4.csv",
    "./Dataset/3.csv",
    "./Dataset/2.csv",
...
PHASES = [
    {
        ...
        'num_labels': 4,
        'ckpt_dirs': [
            "./SBertmodels/AV/checkpoint_dataset1",
            "./SBertmodels/AV/checkpoint_dataset2",
            "./SBertmodels/AV/checkpoint_dataset3",
            "./SBertmodels/AV/checkpoint_dataset4",
            "./SBertmodels/AV/checkpoint_dataset5",
        ],
    },
    {
        'name':       'AC',
        ...
        'num_labels': 2,
        'ckpt_dirs': [
            "./SBertmodels/AC/checkpoint_dataset1",
            "./SBertmodels/AC/checkpoint_dataset2",
            "./SBertmodels/AC/checkpoint_dataset3",
            "./SBertmodels/AC/checkpoint_dataset4",
            "./SBertmodels/AC/checkpoint_dataset5",
        ],
    },
    {
        'name':       'PR',
...
class SBertForClassification(nn.Module):
    ...
    def save_pretrained(self, save_directory):
        """
        Save backbone+config and classifier weights into save_directory.
        """
        os.makedirs(save_directory, exist_ok=True)
        # save transformer backbone + config
        self.backbone.save_pretrained(save_directory)
        # save our classifier head
        torch.save(self.classifier.state_dict(),
                   os.path.join(save_directory, "classifier.pt"))

    @classmethod
    def from_pretrained(cls, load_directory, model_name, num_labels, **kwargs):
        """
        Load backbone+config and classifier from load_directory.
        """
        # init empty
...
def train_and_log(model, train_loader, val_loader, test_loader,
    ...
    for epoch in range(1, epochs+1):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_loss += loss.item()
                preds    = logits.argmax(dim=1)
                vcorrect += (preds == batch['labels']).sum().item()
                vsamples += batch['labels'].size(0)
        val_loss = val_loss / len(val_loader)
        val_acc  = vcorrect / vsamples if vsamples>0 else 0

        # TEST
        test_loss = tcorrect = tsamples = 0
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                test_loss += loss.item()
                preds     = logits.argmax(dim=1)
                tcorrect += (preds == batch['labels']).sum().item()
                tsamples += batch['labels'].size(0)
                all_preds.extend(preds.cpu().tolist())
                all_labels.extend(batch['labels'].cpu().tolist())
        test_loss = test_loss / len(test_loader)
        test_acc  = tcorrect / tsamples if tsamples>0 else 0
    ...
    print(f"Saved per‐epoch metrics to {csv_path}")
    return model, rows
...
if __name__ == "__main__":
    ...
    for phase in PHASES:
        ...
        # save aggregate CSV for phase
        base_dir = os.path.dirname(phase['ckpt_dirs'][0])
        summary_csv = os.path.join(base_dir, f"all_epoch_metrics_{phase_name}.csv")
