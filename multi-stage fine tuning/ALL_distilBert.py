...
def train_and_log(model, train_loader, val_loader, test_loader,
    ...
    ds_csv = os.path.join(
        ckpt_dir,
        f'epoch_metrics_{phase_name}_dataset{ds_idx}.csv'
    )
    ...
    for phase in PHASES:
        ...
        for idx, csv_path in enumerate(DATASET_PATHS):
            ...
            if df[phase['label_col']].isnull().any():
                raise ValueError(f"Unmapped labels in {csv_path} for phase {phase['name']}")
